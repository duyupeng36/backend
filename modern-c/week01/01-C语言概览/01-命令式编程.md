...
C 是命令式编程语言——程序员向计算机下达指令，计算机按顺序逐条执行。每一步是一条明确的命令（语句），程序从 `main` 函数开始，按代码书写的顺序逐步执行。

---

## 是什么

命令式编程（imperative programming）是一种编程范式，程序员通过编写一系列改变程序状态的语句来描述计算过程。C 语言是典型的命令式语言：程序由函数组成，函数由语句组成，语句按顺序执行。每条语句告诉计算机"做这件事"。
...
就像代码示例 `getting-started.c` 中的语句 `printf(...);` 命令计算机向终端输出一行文本。这个特殊的语句是**调用**一个名为`printf`的**函数**，它接收 $4$ 个参数，放在圆括号 `(....)` 中：
...
`getting-started.c` — 这是 Modern C 第一章的完整示例程序。能看懂它，C 语言就正式入门了。
