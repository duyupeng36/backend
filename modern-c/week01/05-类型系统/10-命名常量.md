...
> week01 · C 语言全貌 · 知识点 30

> 状态：☑ 已完成

## 速览

用 `enum`、`constexpr`、`#define` 给字面量命名，消除魔法数。`const` 是运行时只读对象，不是编译时常量。C23 的 `constexpr` 是真正编译时常量。`enum` 用于整数常量，自动递增。两条规则：所有具有特定含义的常量必须命名，不同含义必须区分。

---

## 是什么

本质上来讲，C 程序中只有一种常量就是字面量。但是，在程序中我们不应该直接使用字面量（称为 **魔法数**）；使用语言提供的能力给字面量赋予一个名字可以极大地提高程序的可读性和可扩展性
