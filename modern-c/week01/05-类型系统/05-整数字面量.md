...
+   规则一: 字面量永远非负
    ...
    ```c
    -34 // 不是"负三十四的字面量"，而是"对字面量 34 做一元取负运算"
    ```
...
+   规则三: 十进制整数字面量取第一个能放下的有符号类型。三个有符号类型按 Rank 排列：`signed int → signed long → signed long long`。十进制字面量的类型是第一个能放下它的。假设在某个平台上（`int` 类型占 $16$ 位）：
