...
- [x] **练习 2**：回绕 — `unsigned _BitInt(4) x = 15wbu; x + 1wbu` 的结果是多少？
    答案：`15 + 1` 类型是 `unsigned _BitInt(4)` 结果溢出，回绕到 $0$
---
...
$$
\text{value} = s \cdot 2^{e} \cdot \sum_{k=1}^{p} f_k 2^{-k}
$$

其中 $s$ 是符号（$\pm 1$），$e$ 是指数（$e_{\min} \le e \le e_{\max}$），$f_1 \cdots f_p$ 是尾数位。例如，$s=-1, e=-2, f_1=1, f_2=0, f_3=1$：

$$
-1 \cdot 2^{-2} \cdot (1\cdot 2^{-1} + 0\cdot 2^{-2} + 1\cdot 2^{-3}) = -\frac{5}{32} = -0.15625
$$

这个公式说明：**所有浮点值都可以写成分母是 $2$ 的幂的分数**。这就是十进制小数 $0.1$ 无法精确存储的原因。

> [!TIP]
>
> 浮点数的二进制表示就是在固定宽度的位模式中安排浮点值计算的 $3$ 个部分
>
> + `s` 表示符号（决定正负）：只需要 $1$
> + `e` 表示指数（决定范围）：由浮点格式规定
> + `f` 表示尾数 （决定精度）：由浮点格式规定
...
由于浮点数表示存在的精度问题，**永远不要比较浮点值是否相等**；浮点数之间只能说接近，不能说相等
...
int main(void) {
    // DBL_MIN 是最小正数，不是最负
    printf("DBL_MAX  = %e\n", DBL_MAX);
    printf("DBL_MIN  = %e (最小正数)\n", DBL_MIN);
    printf("-DBL_MAX = %e (最小负数)\n\n", -DBL_MAX);

    // 精度参数
    printf("DBL_MANT_DIG = %d (精度 p)\n", DBL_MANT_DIG);
...
- [x] **练习 1**：DBL_MIN — `DBL_MIN` 和 `-DBL_MAX` 哪个更负？
    答案：`DBL_MIN` 是最小正数，而不是最小值。显然，`-DBL_MAX` 更小

- [x] **练习 2**：精度丢失 — 为什么 `float f = 16777216; f == f + 1` 结果为 true？（提示：`FLT_MANT_DIG = 24`）
    答案：`float` 的位数位数通常是 $24$。$16777216$ 相比较 $1$ 更大，导致较小的精度丢失

- [ ] **练习 3**：完整程序 — 用赫伦法（Heron's method）迭代计算平方根，观察浮点精度导致的收敛行为，保存到 `code/ch05/heron_sqrt.c`
